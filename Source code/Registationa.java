...
class Registationa extends JFrame implements ActionListener {
	...
	public Registationa(User m){
		this.m=m;
		setTitle(" Admin Registation Page");
		...
		l[10]= new JLabel("Welcome to Admin Registation Page");
		l[11]= new JLabel("**( ID must be 1-4 Characters )");
		...
		l[11].setBounds(210+150+30+5,200+30,300,35);
	...
	public void actionPerformed(ActionEvent a){
		...
		String item2=(String)grp.getSelectedItem();
		   ...
		   if(a.getSource()==b2){
			...
			{			   
			if(ui.length()==3||ui.length()==4||ui.length()==1||ui.length()==2){
			if(p1.equals(p2)){
				...
				String dsql="INSERT INTO `admininformation` (`userId`, `firstName`, `lastName`, `password`, `email`, `gender`, `contractNo`,`bloodGroup`, `dateOfBirth`,`department`) VALUES ('"+u.getU()+"', '"+u.getF()+"', '"+u.getL()+"', '"+u.getP()+"', '"+u.getE()+"', '"+u.getG()+"', '"+u.getC()+"', '"+u.getB()+"', '"+u.getD()+"', '"+u.getDp()+"')";
				try{
				...
				// inserting the image on database
				...
				JOptionPane.showMessageDialog(this,"For admin Id must be 1-4 characters");
			}
		}
		   }	
		else{
