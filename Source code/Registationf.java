...
class Registationf extends JFrame implements ActionListener {
	...
	JTextField t13=new JTextField("Available");
	...
	public Registationf(User m){
		
		this.m=m;
		setTitle("Faculty Registation Page");
		...
		l[10]= new JLabel("Welcome to Faculty Registation Page");
		l[12]= new JLabel("Department");
		l[13]= new JLabel("Status");
	    l[11]= new JLabel("**( ID must be 5 0r 6 Characters )");
	  ...
	  public void actionPerformed(ActionEvent a){
		...
		if(a.getSource()==b2){
			...
			if(p1.equals(p2)){
				...
				u.setDp(item);
				u.setS(t13.getText());
				u.setFf(t1.getText()+" "+t2.getText());
				u.setD(yearList.getSelectedItem().toString(),monList.getSelectedItem().toString(),dateList.getSelectedItem().toString());
				
				String dsql="INSERT INTO `facultyinformation` (`userId`, `firstName`, `lastName`, `password`, `email`, `gender`, `contractNo`,`bloodGroup`, `dateOfBirth`,`department`,`status`,`fullName`) VALUES ('"+u.getU()+"', '"+u.getF()+"', '"+u.getL()+"', '"+u.getP()+"', '"+u.getE()+"', '"+u.getG()+"', '"+u.getC()+"', '"+u.getB()+"', '"+u.getD()+"', '"+u.getDp()+"', '"+u.getS()+"', '"+u.getFf()+"')";
				try{
				...
				JOptionPane.showMessageDialog(this,"Faculty Id must be 5-6 characters");
			}
			}	
		}
		else{
